...
const ModernPreviewEditor: React.FC<ModernPreviewEditorProps> = ({
    ...
    const getSpeedAtTime = React.useCallback(
        (time: number): number => {
            ...
            return (
                speedSegments[speedSegments.length - 1]?.speed ?? selectedSpeed
            );
        },
        ...
        (status: any) => {
            if (status.isLoaded && status.durationMillis) {
                ...
                if (speedSegments && videoRef.current) {
                    ...
                    videoRef.current
                        .setRateAsync(initialSpeed, true)
                        .catch(console.warn);
                }
            }
        },
        ...
        (status: any) => {
            ...
            if (status.durationMillis && status.positionMillis !== undefined) {
                ...
                if (
                    ...
                    if (
                        Math.abs(targetSpeed - currentPlaybackRateRef.current) >
                        0.01
                    ) {
                ...
                if (isPlaying && scrollViewRef.current && totalWidth > 0) {
                    ...
                    const scrollX = Math.max(
                        0,
                        progress * totalWidth - SCREEN_WIDTH / 2,
                    );
    ...
    const handleScroll = useCallback(
        (event: any) => {
            ...
            // ONLY update the UI state so the timeline moves smoothly for the user.
            // DO NOT call seekTo() here. Expo-AV cannot handle rapid seek requests.
            setCurrentTime(time);
        },
        [isReady, duration, totalWidth],
    ); // Removed seekTo dependency
    ...
    const handleScrollEnd = useCallback(
        async (event: any) => {
            ...
            // Update UI one last time to be safe
            setCurrentTime(time);

            // Await the actual video seek once the user stops dragging
            await seekTo(time);

            // Add a slight delay before releasing the drag lock.
            // This prevents onPlaybackStatusUpdate from firing an old frame and overriding our new seek position.
            setTimeout(async () => {
    ...
    const handleTextOverlaySave = useCallback(
        (overlay: TextOverlay) => {
            ...
            const existingIndex = existingOverlays.findIndex(
                (o) => o.id === overlay.id,
            );
    ...
    const handleTextOverlayDelete = useCallback(
        (overlayId: string) => {
            const existingOverlays = clip.textOverlays || [];
            const updatedOverlays = existingOverlays.filter(
                (o) => o.id !== overlayId,
            );
            const updatedClip = { ...clip, textOverlays: updatedOverlays };
            onClipUpdate?.(updatedClip);
    ...
    const playheadLeft =
        duration > 0 ? (currentTime / duration) * totalWidth : 0;
        ...
        <View style={styles.container}>
            ...
            <View style={styles.actionButtons}>
                ...
                >
                    <View
                        style={[
                            styles.trimIconContainer,
                            showTrimHandles && styles.trimIconContainerActive,
                        ]}
                    >
                        <Svg
                            width={18}
