          ...
          userRole === "sponsor"
            ? allCompetitions.filter((c: Competition) => {
                ...
                if (!sponsorId) {
                  if (process.env.NODE_ENV === "development") {
                    console.warn(
                      "[Competitions] No sponsor ID for filtering, hiding competition:",
                      c.id
                    );
                  }
                  return false;
                ...
                const compSponsorId =
                  typeof c.sponsorId === "object" && c.sponsorId && "_id" in c.sponsorId
                    ? (c.sponsorId as any)._id
                    : c.sponsorId || "";

                const matches =
                  compSponsorId === sponsorId || compSponsorId === adminData?.sponsorCode;
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              {userRole === "sponsor"
                ...
                : "Manage all competitions and contests"}
            </p>
          </div>
                ...
                {competitions.map((comp) => (
                  <tr key={comp.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span
                        className={`px-1.5 py-0.5 inline-flex text-[10px] leading-4 font-semibold rounded-full ${
                          comp.status === "LIVE"
