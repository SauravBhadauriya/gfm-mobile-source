...
export async function uploadLogo(file: File): Promise<ApiResponse<LogoResponse>> {
  ...
  try {
    ...
    console.log('[Branding API] POST Upload Logo - Status:', httpResponse.status, httpResponse.statusText);
    ...
    try {
      ...
      if (!httpResponse.ok) {
        console.error('[Branding API] POST Upload Logo - Error:', httpResponse.status, httpResponse.statusText);
        return {
          success: false,
...
export async function uploadSplash(files: File[]): Promise<ApiResponse<SplashResponse>> {
  ...
  files.forEach((file, index) => {
    console.log(`[Branding API] POST Upload Splash - File ${index + 1}:`, file.name, `(${file.size} bytes, ${file.type})`);
  });
  ...
  try {
    ...
    console.log('[Branding API] POST Upload Splash - Status:', httpResponse.status, httpResponse.statusText);
    ...
    try {
      ...
      if (!httpResponse.ok) {
        console.error('[Branding API] POST Upload Splash - Error:', httpResponse.status, httpResponse.statusText);
        return {
          success: false,
          message: `Upload failed: ${httpResponse.statusText} (Status: ${httpResponse.status})`,
          error: `HTTP ${httpResponse.status}`,
...
export async function getLogo(): Promise<ApiResponse<LogoResponse>> {
  ...
  try {
    ...
    try {
      ...
      if (!httpResponse.ok) {
        console.error('[Branding API] GET Logo - Error:', httpResponse.status, httpResponse.statusText);
        return {
          success: false,
    ...
    const logoUrl = typeof responseData.data === 'string' ? responseData.data : responseData.data?.logoUrl || '';
...
export async function getSplash(): Promise<ApiResponse<SplashResponse>> {
  ...
  try {
    ...
    console.log('[Branding API] GET Splash - Status:', httpResponse.status, httpResponse.statusText);
    ...
    try {
      ...
      if (!httpResponse.ok) {
        console.error('[Branding API] GET Splash - Error:', httpResponse.status, httpResponse.statusText);
        return {
          success: false,
    ...
    if (!httpResponse.ok || responseData.code !== 1) {
      ...
      return {
        success: false,
        message: responseData.message || `Failed to fetch splash images with status ${httpResponse.status}`,
        error: responseData.error || `HTTP ${httpResponse.status}`,
        data: undefined,
      };
    }
